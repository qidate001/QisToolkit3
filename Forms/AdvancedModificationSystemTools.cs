...
═══════════════════════════════════════════════════════════";
        ...
        private void button_GetPasswordInfo_Click(object sender, EventArgs e)
        {
            ...
            label_Password_UserName.Text = $"用户名：{passwordInfo.UserName ?? "未知"}";
            label_Password_FullName.Text = $"全名：{passwordInfo.FullName ?? "未设置"}";
            label_Password_Description.Text = $"描述：{passwordInfo.Description ?? "未设置"}";

            label_Password_PasswordNeverExpires.Text = $"密码永不过期：{(passwordInfo.PasswordNeverExpires ? "✅ 是" : "❌ 否")}";
            label_Password_UserCannotChangePassword.Text = $"用户不能改密码：{(passwordInfo.UserCannotChangePassword ? "✅ 是" : "❌ 否")}";
            label_Password_PasswordExpired.Text = $"密码已过期：{(passwordInfo.PasswordExpired ? "✅ 是" : "❌ 否")}";
            label_Password_PasswordRequired.Text = $"密码必须存在：{(passwordInfo.PasswordRequired ? "✅ 是" : "❌ 否")}";

            label_Password_AccountEnabled.Text = $"账户已启用：{(passwordInfo.AccountEnabled ? "✅ 是" : "❌ 否")}";
            label_Password_AccountLocked.Text = $"账户已锁定：{(passwordInfo.AccountLocked ? "✅ 是" : "❌ 否")}";
            label_Password_BadPasswordCount.Text = $"错误密码次数：{(passwordInfo.BadPasswordCount)}";
            label_Password_BadLogonCount.Text = $"失败登录次数：{(passwordInfo.BadLogonCount)}";

            label_Password_MaxPasswordAge.Text = $"密码最长有效期：{passwordInfo.MaxPasswordAge?.ToString() ?? "未获取"} 天";
            label_Password_MinPasswordAge.Text = $"密码最短有效期：{passwordInfo.MinPasswordAge?.ToString() ?? "未获取"} 天";
            label_Password_PasswordHistoryLength.Text = $"密码历史记录：{passwordInfo.PasswordHistoryLength?.ToString() ?? "未获取"} 个";
            label_Password_LockoutThreshold.Text = $"账户锁定阈值：{passwordInfo.LockoutThreshold?.ToString() ?? "未获取"} 次失败尝试";
            label_Password_LockoutDuration.Text = $"账户锁定时间：{passwordInfo.LockoutDuration?.ToString() ?? "未获取"} 分钟";

            label_Password_LastPasswordSet.Text = $"上次设置密码：{(passwordInfo.PasswordLastSet.HasValue ?
                passwordInfo.PasswordLastSet.Value.ToString("yyyy-MM-dd HH:mm:ss") : "未知")}";

            label_Password_PasswordExpires.Text = $"密码过期时间：{(passwordInfo.PasswordExpires.HasValue ?
                passwordInfo.PasswordExpires.Value.ToString("yyyy-MM-dd HH:mm:ss") : "未设置 / 永不过期")}";

            label_Password_LastLogin.Text = $"最后登录时间：{(passwordInfo.LastLogin.HasValue ?
                passwordInfo.LastLogin.Value.ToString("yyyy-MM-dd HH:mm:ss") : "从未登录")}";

            label_Password_LastBadLoginAttempt.Text = $"密码过期时间：{(passwordInfo.LastBadLoginAttempt.HasValue ?
                passwordInfo.LastBadLoginAttempt.Value.ToString("yyyy-MM-dd HH:mm:ss") : "无失败记录")}";

            label_Password_AccountExpirationDate.Text = $"账户过期时间：{(passwordInfo.AccountExpirationDate.HasValue ?
                passwordInfo.AccountExpirationDate.Value.ToString("yyyy-MM-dd HH:mm:ss") : "永不过期")}";
